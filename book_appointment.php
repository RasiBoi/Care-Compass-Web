...
$booked_sql = "SELECT schedule_id FROM bookings WHERE doctor_id = $doctor_id";
$booked_result = $conn->query($booked_sql);
$booked_slots = [];
if ($booked_result->num_rows > 0) {
    while ($booked_row = $booked_result->fetch_assoc()) {
        $booked_slots[] = $booked_row['schedule_id'];
    }
}

// Fetch doctor's timetable
$timetable_sql = "SELECT * FROM timetable WHERE doctor_id = $doctor_id ORDER BY FIELD(day, 'Sunday', 'Monday', 'Tuesday', 'Wednesday', 'Thursday', 'Friday', 'Saturday'), time_slot";
...
<?php endif; ?>
    ...
    <div class="schedule-container">
        <a href="search_results.php" class="go-back">Go Back</a>

        <?php if ($doctor): ?>
            <div class="doctor-info">
                <img src="<?= htmlspecialchars($doctor['image_path']); ?>" alt="Doctor Image">
                <h1><?= htmlspecialchars($doctor['name']); ?></h1>
                <h3><?= htmlspecialchars($doctor['specialty']); ?></h3>
                <p><?= htmlspecialchars($doctor['clinic_address']); ?></p>
            </div>
                ...
                <tbody>
                    <?php if ($timetable_result->num_rows > 0): ?>
                        <?php while ($row = $timetable_result->fetch_assoc()): ?>
                            <?php $is_available = !in_array($row['id'], $booked_slots); ?>
                            <tr>
                                <td><?= date('d-M-Y', strtotime("next " . $row['day'])); ?></td>
                                <td><?= htmlspecialchars($row['day']); ?></td>
                                <td><?= htmlspecialchars($row['time_slot']); ?></td>
                                <td><?= $is_available ? '01' : 'N/A'; ?></td>
                                <td class="<?= $is_available ? 'availability' : 'not-available'; ?>">
                                    <?= $is_available ? 'Available' : 'Not Available'; ?>
                                </td>
                                <td>
                                    <?php if ($is_available): ?>
                                        <button class="btn" onclick="bookAppointment(<?= $row['id']; ?>)">Book Now</button>
                                    <?php else: ?>
                    ...
                    <?php else: ?>
                        <tr>
                            <td colspan="6">No schedule available for this doctor.</td>
                        </tr>
                    <?php endif; ?>
                </tbody>
            </table>
...
function bookAppointment(scheduleId) {
            alert("Booking initiated for schedule ID: " + scheduleId);
            window.location.href = `confirm_booking.php?schedule_id=${scheduleId}`;
        }
    </script>                    
